...
function cropCard(item, type) {
  ...
  const action = isTornado || isEmpty ? '' : type === 'ready' ? '<button type="button" data-ui-action="harvest">Thu hoạch</button>' : item.fertilised ? '' : cropFertiliserIcons.map((icon, index) => `<button class="fertiliser-button" type="button" data-ui-action="fertilise" data-fertiliser-index="${index}" title="Bón phân ${index + 1}"><img src="${icon}" alt="Phân bón ${index + 1}" /><span>×${formatExactCount(fertiliserCounts.get(icon))}</span></button>`).join('');
  if (isEmpty) {
    const plantSeed = item.canPlant ? `<button type="button" class="empty-crop-plant" data-ui-action="plant" data-selected-seed="${escapeHtml(item.seedName || '')}" data-target-fertiliser-type="${item.fertiliserType || 0}" data-action-label="${plantLabel}"${plantCount ? '' : ' disabled'}>${plantLabel}</button>` : '';
    const emptyOverlay = plantSeed ? `<div class="empty-crop-overlay">${plantSeed}</div>` : '';
    return `<article class="crop-card is-empty empty-crop-card" data-crop-name="${escapeHtml(item.label)}" data-fertiliser-type="${item.fertiliserType || 0}" data-map-keys="${escapeHtml((item.mapKeys || []).join('||'))}" data-count="${item.count}"><div class="crop-icon-box"><img class="crop-image" src="${escapeHtml(item.icon)}" alt="" /><b class="crop-quantity">×${item.count}</b></div><div class="crop-card-content"><span class="crop-card-state">Đất trống</span><strong class="crop-card-title">${escapeHtml(item.label)}</strong></div>${emptyOverlay}</article>`;
  }
